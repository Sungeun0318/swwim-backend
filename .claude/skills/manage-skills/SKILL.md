...
|------|------|---------------|
| `verify-build` | Flutter analyze + Backend 컴파일 검증 | `pubspec.yaml`, `analysis_options.yaml`, `lib/**/*.dart`, `swwim-backend/build.gradle.kts`, `swwim-backend/src/**/*.java` |
| `verify-api-sync` | 프론트/백엔드 API 엔드포인트 일관성 검증 | `lib/core/api/services/*_api_service.dart`, `swwim-backend/src/**/controller/*.java`, `swwim-backend/src/**/dto/**/*.java` |
| `verify-auth` | JWT 인증 플로우 전체 검증 | `lib/core/auth/*.dart`, `lib/core/api/api_client.dart`, `lib/main.dart`, `swwim-backend/src/**/security/*.java`, `swwim-backend/src/**/controller/AuthController.java` |
| `verify-deploy` | EC2 배포 전 사전 검증 | `swwim-backend/build.gradle.kts`, `swwim-backend/src/main/resources/*.properties`, `swwim-backend/src/**/entity/**/*.java` |
